...
class ECN_PixelPose(nn.Module):
    def __init__(self, input_size, nb_ref_imgs=2, in_planes=3,init_planes=16, scale_factor=0.5, growth_rate=16,
        ...
        self.nb_ref_imgs = nb_ref_imgs
        self.pred_planes = 1+6      # 最终的预测通道数

        in_planes = (1 + nb_ref_imgs) *in_planes  # (1+4) * 3 

        out_planes = init_planes  # 16
        output_size = input_size  # 260
        ...
        print(len(self.encoding_layers), ' encoding layers.')  # 4 layers
        print(out_planes, ' encoded feature maps.')   # 16-> [32, 48, 64, 80]
        ...
        in_planes2 = out_planes  # encoder planes  80
        self.predicts = 50
        planes = []
        ...
        print(len(self.decoding_layers), ' decoding layers.')  #5
        print(out_planes, ' decoded feature maps.')# [64,48,32,16,15]
    ...
    def forward(self, target_image, ref_imgs):
        ...
        input = [target_image]
        input.extend(ref_imgs)
        input = torch.cat(input, 1)  # b,15,480,640

        b, _, h, w = input.shape
        encode = [input]
        encode.append(self.conv1(encode[-1]))  # b, 16,240,320

        for i, layer in enumerate(self.encoding_layers):
            encode.append(layer(encode[-1])) # encode[-1]: b, 16,240,320;    layer(): b,32,120,160

        out = encode[-1]  # b,80,15,20
        
        pose = self.pose_pred(out)   # b,6,15,20
        pose = pose.mean(3).mean(2)   # b,6   对第 3 维（宽度维度）进行平均池化操作(b,6,15), 对第 2 维（高度维度）进行平均池化操作，即将每个通道上的每一行的平均值再取平均
        pose = 0.01 * pose.view(pose.size(0), 1, 6)  # b,1,6     Our network outputs a pose vector p, given by a 3D camera position x and orientation represented by quaternion q:
        ...
        exps = [torch.sigmoid(predicts[i][:, :1]) for i in range(self.predicts)]  # b,1,480,640
        #print('exp shape', np.shape(exps[0]))
        # for i in range(self.predicts):
        #     exps[i][exps[i]>=0.3] = 1.0
        #     exps[i][exps[i]<0.3] = 0.0
        
        #print('exps0',torch.max(exps[0]))
        res_pose = [0.001 * predicts[i][:, 1:] for i in range(self.predicts)]  # b,6,480,640

        for i in range(self.predicts):
            res_pose[i][:,3:]=0.   # without rotation
...
class ECN_Pose(nn.Module):
    ...
    def forward(self, target_image, ref_imgs):
        ...
        pose = self.pose_pred(out)   # b, 6*n_motions, 15, 20
        pose = pose.mean(3).mean(2)
        pose = 0.01 * pose.view(pose.size(0), self.n_motions,1,1,6)  # b,4,1,1,6
        if self.n_motions>1:
            pose[:, 1:,:,:,3:]=0.
            #print(pose[0])
        ...
        #print(predicts)
        exps = [F.softmax(predicts[i],dim=1) for i in range(self.predicts)]   # b,4,480,640
        #print('exp = ',np.shape(exps[0]))   # 5* (b,4,480,640)
        #print(np.sum(exps[0].cpu().data.numpy()[0,:,0,0]))  # The mixture weights sum to 1 for each pixel
        ...
        # unsqueeze(4) 会将指定维度dim=4的大小增加为1，从而扩展张量的维度 
        final_pose=[torch.sum(pose*exps[i].unsqueeze(4),dim=1).permute(0,3,1,2) for i in range(self.predicts)]  # b,6,480,640
        
        ego_pose=pose[:,0].view(-1,1,6)   # b,1,6, first pose
        if self.training:
...
class ECN_Pose2(nn.Module):
    def __init__(self, input_size, nb_ref_imgs=2, init_planes=16, scale_factor=0.5, growth_rate=16,
        ...
        if self.output_exp:
            ...
            self.predicts=len(planes)
            #print('111111111111predicts_layers2 = ', self.predicts)
            self.predict_maps = nn.ModuleList()
            for i in range(self.predicts):
                if False:
